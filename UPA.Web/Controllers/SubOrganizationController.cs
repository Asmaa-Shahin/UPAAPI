...
namespace UPA.Web.Controllers
{
    ...
    public class SubOrganizationController : ControllerBase
    {
        ...
        {
            return _SubOrganizationService.ListSubOrganizations();
        }
        ...
        public IActionResult Update(EditSubOrganizationVM subOrganizationVM)
        {
            try
            {
                ...
                var lstOrgCode = _SubOrganizationService.ListSubOrganizations().ToList().Where(a => a.Code == subOrganizationVM.Code && a.Id != id).ToList();
                if (lstOrgCode.Count > 0)
                {
                    return StatusCode(StatusCodes.Status500InternalServerError, new ErrorResponse { Status = "code", Message = "Sub Organization code already exist", MessageAr = "هذا الكود مسجل سابقاً" });
                }
                var lstOrgNames = _SubOrganizationService.ListSubOrganizations().ToList().Where(a => a.Name == subOrganizationVM.Name && a.Id != id).ToList();
                if (lstOrgNames.Count > 0)
                {
                    return StatusCode(StatusCodes.Status500InternalServerError, new ErrorResponse { Status = "name", Message = "Sub Organization name already exist", MessageAr = "هذا الاسم مسجل سابقاً" });
                }



                var lstOrgArNames = _SubOrganizationService.ListSubOrganizations().ToList().Where(a => a.NameAr == subOrganizationVM.NameAr && a.Id != id).ToList();
                if (lstOrgArNames.Count > 0)
                {
                    return StatusCode(StatusCodes.Status500InternalServerError, new ErrorResponse { Status = "nameAr", Message = "Sub Organization arabic name already exist", MessageAr = "هذا الاسم مسجل سابقاً" });
                }
        ...
        [HttpGet]
        [Route("GenerateSubcode")]
        public GeneratedAssetDetailBCVM GenerateSubcode()
        {
            return _SubOrganizationService.GenerateSubcode();
        }
        [HttpPost]
        [Route("AddSubOrganization")]
        public ActionResult<SubOrganization> Add(CreateSubOrganizationVM subOrganizationVM)
        {
            var lstOrgCode = _SubOrganizationService.ListSubOrganizations().ToList().Where(a => a.Code == subOrganizationVM.Code).ToList();
            if (lstOrgCode.Count > 0)
            {
                return StatusCode(StatusCodes.Status500InternalServerError, new ErrorResponse { Status = "code", Message = "Sub Organization code already exist", MessageAr = "هذا الكود مسجل سابقاً" });
            }
            if (subOrganizationVM.Code?.Length > 5)
            {
            ...
            var lstOrgNames = _SubOrganizationService.ListSubOrganizations().ToList().Where(a => a.Name == subOrganizationVM.Name).ToList();
            if (lstOrgNames.Count > 0)
            {
                return StatusCode(StatusCodes.Status500InternalServerError, new ErrorResponse { Status = "name", Message = "Sub Organization name already exist", MessageAr = "هذا الاسم مسجل سابقاً" });
            }

            var lstOrgArNames = _SubOrganizationService.ListSubOrganizations().ToList().Where(a => a.NameAr == subOrganizationVM.NameAr).ToList();
            if (lstOrgArNames.Count > 0)
            {
                return StatusCode(StatusCodes.Status500InternalServerError, new ErrorResponse { Status = "nameAr", Message = "Sub Organization arabic name already exist", MessageAr = "هذا الاسم مسجل سابقاً" });
            }
        ...
        public ActionResult<SubOrganization> Delete(int id)
        {
            try
            {
                ...
                {
                    return StatusCode(StatusCodes.Status500InternalServerError, new ErrorResponse { Status = "hospital", Message = "You cannot delete this sub organization it has related hospitals", MessageAr = "لا يمكنك مسح الهيئة الفرعية وذلك لارتباط مستشفيات  بها" });
                }
                else
                {
