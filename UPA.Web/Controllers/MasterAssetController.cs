...
namespace UPA.Web.Controllers
{
    ...
    public class MasterAssetController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("SortMasterAssetafterSearch/{pageNumber}/{pageSize}")]
        public IndexMasterAssetVM SortMasterAssetafterSearch(FilterAndSortVM data, int pageNumber, int pageSize)
        {

            return _masterAssetService.SortMasterAssetafterSearch(data, pageNumber, pageSize);
        }
        ...
        public ActionResult Add(CreateMasterAssetVM MasterAssetVM)
        {
            ...
            var lstNames = _masterAssetService.ListMasterAsset().Results?.ToList().Where(a =>( a.Name == MasterAssetVM.Name || a.NameAr==MasterAssetVM.NameAr  )&& MasterAssetVM.BrandId ==a.BrandId   ).ToList();
            if (lstNames?.Count > 0)
            {
