...
{
    public class SubOrganizationRepository : ISubOrganizationService
    {
        private UPAModel _context;


        public SubOrganizationRepository(UPAModel context)
        {
            _context = context;
        }
        ...
        public GeneratedAssetDetailBCVM GenerateSubcode()
        {
            ...
            var lastId = _context.SubOrganizations.ToList();
            if (lastId.Count > 0)
            {
                var code = lastId.LastOrDefault()?.Code;



                var barcode = (int.Parse(code)+1).ToString();
                var lastcode = barcode.ToString().PadLeft(3, '0');
        ...
        public int Add(CreateSubOrganizationVM subOrganizationVM)
        {
            ...
            try
            {
                if (subOrganizationVM != null)
                {
                    ...
                    SubOrganizationObj.OrganizationId =(int) subOrganizationVM.OrganizationId;
        ...
        public IEnumerable<IndexSubOrganizationVM.GetData> ListSubOrganizations()
        {
            var lstSubOrganizations = _context.SubOrganizations.ToList().Select(item => new IndexSubOrganizationVM.GetData
            {
                Id = item.Id,
