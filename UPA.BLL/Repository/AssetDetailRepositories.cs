...
;
            ...
            if (lstAllAssets.Count > 0)
            {
                foreach (var asset in lstAllAssets)
                {
                    ...
                    detail.MasterAssetId = asset.MasterAssetId;
                    detail.PurchaseDate = asset.PurchaseDate;
                    detail.HospitalId = asset.Hospital!=null ? asset.Hospital.Id:0;
                    detail.HospitalName = asset.Hospital!=null ? asset.Hospital.Name:"";
                    detail.HospitalNameAr = asset.Hospital != null ? asset.Hospital.NameAr:"";
                    detail.AssetName = asset.MasterAsset.Name;
                    detail.AssetNameAr = asset.MasterAsset.NameAr;
                  
                    detail.GovernorateId = asset.Hospital != null ? asset.Hospital.GovernorateId:0;
                    detail.GovernorateName = asset.Hospital != null ? asset.Hospital.Governorate.Name:"";
                    detail.GovernorateNameAr = asset.Hospital != null ? asset.Hospital.Governorate.NameAr:"";
                    detail.CityId = asset.Hospital != null ? asset.Hospital.CityId:0;
                    detail.CityName = asset.Hospital!=null? (asset.Hospital.City != null ? asset.Hospital.City.Name : ""):"";
                    detail.CityNameAr = asset.Hospital != null ? (asset.Hospital.City != null ? asset.Hospital.City.NameAr : "") : "";
                    detail.OrganizationId = asset.Hospital != null ?asset.Hospital.OrganizationId:0;
                    detail.OrgName = asset.Hospital != null?asset.Hospital.Organization.Name:"";
                    detail.OrgNameAr = asset.Hospital != null ? asset.Hospital.Organization.NameAr:"";
                    detail.SubOrganizationId = asset.Hospital != null ? asset.Hospital.SubOrganizationId:0;
                    detail.SubOrgName = asset.Hospital!=null?(asset.Hospital.SubOrganization!=null ? asset.Hospital.SubOrganization.Name:""):"";
                    detail.SubOrgNameAr = asset.Hospital != null ? (asset.Hospital.SubOrganization != null ? asset.Hospital.SubOrganization.NameAr : "") : "";
                    detail.SupplierName = asset.Supplier != null ? asset.Supplier.Name : "";
                    detail.SupplierNameAr = asset.Supplier != null ? asset.Supplier.NameAr : "";
                    detail.QrFilePath = asset.QrFilePath;
        ...
        public List<DrawChart> DrawingChart()
        {
            ...
            if (lstHospital.Count() > 0)
            {
                foreach (var host in lstHospital)
                {
                    ...
                    //Key
                    var orgId = chartObj.OrganizationId = host.FirstOrDefault().Organization.Id;
                    ...
                    var lstGovs = host.GroupBy(c => c.GovernorateId).ToList();


                    List<DrawBarChart> listBars = new List<DrawBarChart>();
                    var count = lstGovs.Count();
                    foreach (var gov in lstGovs)
                    {
                        
                        DrawBarChart barObj = new DrawBarChart();
                        var CountOfAsset = _context.AssetDetails.Include(a => a.Hospital).Where(a => a.Hospital.GovernorateId == gov.FirstOrDefault().GovernorateId && a.Hospital.OrganizationId == orgId).Count();
                        barObj.AssetCount = CountOfAsset;
                        barObj.GovernorateId = (int)gov.FirstOrDefault().GovernorateId;
                        barObj.GovernorateName = gov.FirstOrDefault().Governorate.Name;
                        barObj.GovernorateNameAr = gov.FirstOrDefault().Governorate.NameAr;
                        listBars.Add(barObj);
                    }
        ...
        {
            List<DrawChartByGov> list = new List<DrawChartByGov>();
         
            var lstgov = _context.Hospitals.Include(a => a.Governorate).Include(a => a.Organization).ToList().GroupBy(c => c.GovernorateId);

            if (lstgov.Count() > 0)
            {
                foreach (var gov in lstgov)
                {
                    DrawChartByGov chartObj = new DrawChartByGov();
                    //Key
                    var govId = chartObj.GovernorateId = gov.FirstOrDefault().Governorate.Id;
                    chartObj.GovernorateName = gov.FirstOrDefault().Governorate.Name;
                    chartObj.GovernorateNameAr = gov.FirstOrDefault().Governorate.NameAr;


                    var lstGovs = gov.GroupBy(c => c.OrganizationId).ToList();


                    List<DrawBarChartByGov> listBars = new List<DrawBarChartByGov>();
                    var count = lstGovs.Count();
                    foreach (var org in lstGovs)
                    {
                        DrawBarChartByGov barObj = new DrawBarChartByGov();
                        var CountOfAsset = _context.AssetDetails.Include(a => a.Hospital).Where(a => a.Hospital.OrganizationId == org.FirstOrDefault().OrganizationId && a.Hospital.GovernorateId == govId).Count();
                        barObj.AssetCount = CountOfAsset;
                        barObj.OrganizationId = (int)org.FirstOrDefault().OrganizationId;
                        barObj.OrganizationName = org.FirstOrDefault().Organization.Name;
                        barObj.OrganizationNameAr = org.FirstOrDefault().Organization.NameAr;
                        listBars.Add(barObj);
                    }
                    chartObj.ListBars = listBars;
                    list.Add(chartObj);
        ...
        public List<DrawChartForGovAndPopulation> DrawChartForGovAndPopulation(ChartFilter? filter)
        {
           var lstgov = _context.Governorates.ToList();
            List<DrawChartForGovAndPopulation> lstGovAndPopulations = new List<DrawChartForGovAndPopulation>();
            if (lstgov.Count()>0)
            {
                foreach (var item in lstgov)
                {
                    DrawChartForGovAndPopulation obj = new DrawChartForGovAndPopulation();
                    obj.GovernorateName = item.Name;
                    obj.GovernorateNameAr = item.NameAr;
                    IQueryable<AssetDetail> assetList = _context.AssetDetails
                    .Include(a => a.Hospital).Include(a => a.MasterAsset);
                    if (filter.BrandIds.Count > 0)
                    {
                        assetList = assetList.Where(a => a.Hospital.GovernorateId == item.Id && filter.BrandIds.Contains(a.MasterAsset!=null? a.MasterAsset.BrandId:0));
                    }
                    if (filter.CategoryIds.Count > 0)
                    {
                        assetList = assetList.Where(a => a.Hospital.GovernorateId == item.Id && filter.CategoryIds.Contains(a.MasterAsset != null ? a.MasterAsset.CategoryId : 0));
                    }
                    if (filter.OrganizationIds.Count > 0)
                    {
                        assetList = assetList.Where(a => a.Hospital.GovernorateId == item.Id && filter.OrganizationIds.Contains(a.Hospital != null ? a.Hospital.OrganizationId : 0));
                    }
                    else
                    {
                        assetList = assetList.Where(a => a.Hospital.GovernorateId == item.Id);
                    }


                    decimal NumberOfAsset = assetList.Count();

                    if (NumberOfAsset > 0)
                    {
                        obj.NumberOfPopulationsforEachAsset = item.Population / NumberOfAsset;
                    }
                    else
                    {
                        obj.NumberOfPopulationsforEachAsset = 0;
                    }
                   



                    lstGovAndPopulations.Add(obj);
            ...
            return lstGovAndPopulations.OrderBy(x => x.NumberOfPopulationsforEachAsset).ToList() ;
        }
        ...
        public IEnumerable<IndexAssetDetailVM.GetData> GetAll()
        {
            ...
            if (lstAssetDetails.Count > 0)
            {
                foreach (var asset in lstAssetDetails)
                {
                    ...
                    item.CityName = asset.Hospital.City !=null ? asset.Hospital.City.Name : "";
                    item.CityNameAr = asset.Hospital.City != null ? asset.Hospital.City.NameAr : "";
        ...
        public IndexAssetDetailVM SortAssetsafterSearch(FilterAndSortVMAssets data, int pageNumber, int pageSize)
        {
            ...
            IQueryable<AssetDetail> query = _context.AssetDetails.Include(a => a.Hospital).Include(a => a.MasterAsset).Include(a=>a.Supplier).Include(a=>a.MasterAsset.AssetPeriority);
            IndexAssetDetailVM mainClass = new IndexAssetDetailVM();
            List<IndexAssetDetailVM.GetData> list = new List<IndexAssetDetailVM.GetData>();
            List<AssetDetail> searchResult = new List<AssetDetail>();
            if (data.SearchBy != null)
            {
                if (data.SearchBy.Name.Count > 0)
                {
                    query = query.Where(a => data.SearchBy.Name.Contains(a.MasterAsset.Name) || data.SearchBy.Name.Contains(a.MasterAsset.NameAr));
                }
                if (data.SearchBy.GovId.Count > 0)
                {
                    query = query.Where(a => data.SearchBy.GovId.Contains(a.Hospital.GovernorateId));
                }
                if (data.SearchBy.BrandId.Count > 0)
                {
                    query = query.Where(a => data.SearchBy.BrandId.Contains(a.MasterAsset.BrandId));
                }
                if (data.SearchBy.Model.Count > 0)
                {
                    query = query.Where(a => data.SearchBy.Model.Contains(a.MasterAsset.ModelNumber));
                }
                if (data.SearchBy.HospitalId.Count > 0)
                {
                ...
                if (data.SearchBy.SubCategoryId.Count > 0)
                {
                ...
                DateTime? startingFrom = new DateTime();
                DateTime? endingTo = new DateTime();
                if (data.SearchBy.Start == "")
                {
                    data.SearchBy.purchaseDateFrom = DateTime.Parse("01/01/1900");
                    startingFrom = DateTime.Parse("01/01/1900");
                }
                else
                {
                    data.SearchBy.purchaseDateFrom = DateTime.Parse(data.SearchBy.Start.ToString());
                    var startyear = data.SearchBy.purchaseDateFrom.Value.Year;
                    var startmonth = data.SearchBy.purchaseDateFrom.Value.Month;
                    var startday = data.SearchBy.purchaseDateFrom.Value.Day;
                    if (startday < 10)
                        setstartday = data.SearchBy.purchaseDateFrom.Value.Day.ToString().PadLeft(2, '0');
                    else
                        setstartday = data.SearchBy.purchaseDateFrom.Value.Day.ToString();

                    if (startmonth < 10)
                        setstartmonth = data.SearchBy.purchaseDateFrom.Value.Month.ToString().PadLeft(2, '0');
                    else
                        setstartmonth = data.SearchBy.purchaseDateFrom.Value.Month.ToString();

                    var sDate = startyear + "-" + setstartmonth + "-" + setstartday;
                    startingFrom = DateTime.Parse(sDate);//.AddDays(1);
                }

                if (data.SearchBy.End == "" || data.SearchBy.End==null)
                {
                    data.SearchBy.purchaseDateTo = DateTime.Today.Date;
                    endingTo = DateTime.Today.Date;
                }
                else
                {
                    data.SearchBy.purchaseDateTo = DateTime.Parse(data.SearchBy.End.ToString());
                    var endyear = data.SearchBy.purchaseDateTo.Value.Year;
                    var endmonth = data.SearchBy.purchaseDateTo.Value.Month;
                    var endday = data.SearchBy.purchaseDateTo.Value.Day;
                    if (endday < 10)
                        setendday = data.SearchBy.purchaseDateTo.Value.Day.ToString().PadLeft(2, '0');
                    else
                        setendday = data.SearchBy.purchaseDateTo.Value.Day.ToString();
                    if (endmonth < 10)
                        setendmonth = data.SearchBy.purchaseDateTo.Value.Month.ToString().PadLeft(2, '0');
                    else
                        setendmonth = data.SearchBy.purchaseDateTo.Value.Month.ToString();
                    var eDate = endyear + "-" + setendmonth + "-" + setendday;
                    endingTo = DateTime.Parse(eDate);
                }
                if (startingFrom != null || endingTo != null)
                {
                       ...
                       query = query.Where(a => a.InstallationDate.Value.Date >= startingFrom.Value.Date && a.InstallationDate.Value.Date <= endingTo.Value.Date);
                ...
                switch (data.SortBy)
                {
                    ...
                    case "Asset Name":
                        if (data.SortStatus == "ascending")
                        {
                            ...
                            query = query.OrderBy(ww => ww.MasterAsset.Name);
                    ...
                    case "Governorate":
                        ...
                        {
                            query = query.
                                 OrderBy(ww => ww.Hospital.Governorate.Name);
                        }
                        else
                        {
                            query = query.
                                 OrderByDescending(ww => ww.Hospital.Governorate.Name);
                        }
                        break;
                    case "المحافظة":
                        if (data.SortStatus == "ascending")
                        {
                            query = query.
                                 OrderBy(ww => ww.Hospital.Governorate.NameAr);
                        }
                        else
                        {
                            query = query.
                                 OrderByDescending(ww => ww.Hospital.Governorate.NameAr);
                        }
                        break;
                    ...
                    case "Hospital":
                        ...
                        if (data.SortStatus == "ascending")
                        {
                            query = query.OrderBy(ww => ww.Hospital.Name);
                        }
                        ...
                        {
                            query = query.
                                 OrderByDescending(ww => ww.Hospital.Name);
                        }
                    ...
                    case "المستشفى":
                        if (data.SortStatus == "ascending")
                        {
                            query = query.OrderBy(ww => ww.Hospital.NameAr);
                        }
                        else
                        {
                            query = query.OrderByDescending(ww => ww.Hospital.NameAr);
                        }
                    ...
                    case "Brand":
                        ...
                        if (data.SortStatus == "ascending")
                        {
                            ...
                            query = query.OrderBy(ww => ww.MasterAsset.Brand.Name);
                        }
                        else
                        {
                            query = query.OrderByDescending(ww => ww.MasterAsset.Brand.Name);
                        }
                    ...
                    case "الماركة":
                        ...
                        if (data.SortStatus == "ascending")
                        {
                            ...
                            query = query.OrderBy(ww => ww.MasterAsset.Brand.NameAr);
                        }
                        else
                        {
                            query = query.OrderByDescending(ww => ww.MasterAsset.Brand.NameAr);
                        }
                    ...
                    case "Category":
                        ...
                        if (data.SortStatus == "ascending")
                        {
                            ...
                            query = query.OrderBy(ww => ww.MasterAsset.Category.Name);
                        }
                        else
                        {
                            query = query.OrderByDescending(ww => ww.MasterAsset.Category.Name);
                        }
                    ...
                    case "التصنيف":
                        ...
                        if (data.SortStatus == "ascending")
                        {
                            ...
                            query = query.OrderBy(ww => ww.MasterAsset.Category.NameAr);
                        }
                        else
                        {
                            query = query.OrderByDescending(ww => ww.MasterAsset.Category.NameAr);
                        }
            ...
            if (pageNumber > 0 && pageSize > 0)
            {
                ...
                list = query.Skip((pageNumber - 1) * pageSize).Take(pageSize).Select(a => new IndexAssetDetailVM.GetData
                {
                    Id = a.Id,
            ...
            {
                list = query.Select(a => new IndexAssetDetailVM.GetData
                {
                    Id = a.Id,
                    AssetName = a.MasterAsset.Name,
                    AssetNameAr = a.MasterAsset.NameAr,
        ...
        public List<HospitalAssetAge> GetAssetsByAgeGroup(ChartFilter filter)
        {
           ...
           IQueryable<AssetDetail>   assetList = _context.AssetDetails.Include(a => a.Hospital).Include(a => a.MasterAsset).Where(a => a.InstallationDate != null);

            if (filter.BrandIds.Count > 0)
            {
                assetList = assetList.Where(a => filter.BrandIds.Contains(a.MasterAsset.BrandId));
            }
            if (filter.CategoryIds.Count > 0)
            {
                assetList = assetList.Where(a => filter.CategoryIds.Contains(a.MasterAsset.CategoryId));
            }
            if (filter.OrganizationIds.Count > 0)
            {
                assetList = assetList.Where(a => filter.OrganizationIds.Contains(a.Hospital.OrganizationId));
            }
           var  assetListAfterGroupBy = assetList
            .GroupBy(p => (DateTime.Today.Year - p.InstallationDate.Value.Year) / 5).ToList();

            lstHospitalAssets = assetListAfterGroupBy
                    .OrderBy(p => p.Key)
                            .Select(gr => new HospitalAssetAge
                            {
                                AgeGroup = String.Format("{0}-{1}", gr.Key * 5, (gr.Key + 1) * 5),
