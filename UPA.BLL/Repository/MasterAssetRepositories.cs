...
namespace UPA.BLL.Repository
{
    public class MasterAssetRepositories:  IMasterAssetService
    {
        ...
        public ViewMasterAssetVM ViewMasterAsset(int id)
        {
            ...
            var lstOrigins = _context.Origins.Where(a => a.Id == masterAssetObj.OriginId).ToList();
            if (lstOrigins.Count() > 0)
            {
        ...
        public IndexMasterAssetVM SortMasterAssetafterSearch(FilterAndSortVM data, int pageNumber, int pageSize)
        {


            IQueryable<MasterAsset> query = _context.MasterAssets.Include(a=>a.AssetPeriority);
            IndexMasterAssetVM mainClass = new IndexMasterAssetVM();
            List<IndexMasterAssetVM.GetData> list = new List<IndexMasterAssetVM.GetData>();
            List<MasterAsset> searchResult = new List<MasterAsset>();
            if (data.SearchBy != null)
            {
                if (data.SearchBy.Name.Count > 0)
                {
                    query = query.Where(a => data.SearchBy.Name.Contains(a.Name) || data.SearchBy.Name.Contains(a.NameAr));
                }
                if (data.SearchBy.Code.Count > 0)
                {
                    query = query.Where(a => data.SearchBy.Code.Contains(a.Code));
                }
                if (data.SearchBy.BrandId.Count > 0)
                {
                    query = query.Where(a => data.SearchBy.BrandId.Contains(a.BrandId));
                }
                if (data.SearchBy.Model.Count > 0)
                {
                    query = query.Where(a => data.SearchBy.Model.Contains(a.ModelNumber));
                }
                if (data.SearchBy.OriginId.Count > 0)
                {
                    query = query.Where(a => data.SearchBy.OriginId.Contains(a.OriginId));
                }
                if (data.SearchBy.ECRIId.Count > 0)
                {
                    query = query.Where(a => data.SearchBy.ECRIId.Contains(a.ECRIId));
                }
                if (data.SearchBy.AssetPeriorityId.Count > 0)
                {
                    query = query.Where(a => data.SearchBy.AssetPeriorityId.Contains(a.PeriorityId));
                }
            }
                    ...
                    switch (data.SortBy)
                    {
                        ...
                        case "Name":
                            if (data.SortStatus == "ascending")
                            {
                                ...
                                query = query.OrderBy(ww => ww.Name);
                        ...
                        case "Model":
                            ...
                            else
                            {
                               ...
                               OrderByDescending(ww => ww.ModelNumber);
                            }
                        ...
                        case "Periority":
                            ...
                            {
                                query = query.
                                     OrderBy(ww => ww.AssetPeriority.Name);
                            }
                            else
                            {
                                query = query.
                                     OrderByDescending(ww => ww.AssetPeriority.Name);
                            }
                            break;
                        case "الأولوية":
                            if (data.SortStatus == "ascending")
                            {
                                query = query.
                                     OrderBy(ww => ww.AssetPeriority.NameAr);
                            }
                            else
                            {
                                query = query.
                                     OrderByDescending(ww => ww.AssetPeriority.NameAr);
                            }
                            break;
                        ...
                        case "ECRI":
                            ...
                            if (data.SortStatus == "ascending")
                            {
                                query = query.OrderBy(ww => ww.ECRIS.Name);
                            }
                            ...
                            {
                                query = query.
                                     OrderByDescending(ww => ww.ECRIS.Name);
                            }
                        ...
                        case "الماركة":
                            if (data.SortStatus == "ascending")
                            {
                                query = query.OrderBy(ww => ww.Brand.Name);
                            }
                            else
                            {
                                query = query.OrderByDescending(ww => ww.Brand.Name);
                            }
                        ...
                        case "Brand":
                            ...
                            if (data.SortStatus == "ascending")
                            {
                                ...
                                query = query.OrderBy(ww => ww.Brand.NameAr);
                            }
                            else
                            {
                                query = query.OrderByDescending(ww => ww.Brand.NameAr);
                            }
            ...
            if (pageNumber>0 && pageSize > 0 )
            {
                ...
                list = query.Skip((pageNumber - 1) * pageSize).Take(pageSize).Select(a => new IndexMasterAssetVM.GetData
                {
                    Id=a.Id,
                    Name = a.Name,
                    NameAr = a.NameAr,
                    BrandName = a.Brand.Name,
                    BrandNameAr = a.Brand.NameAr,
                    Code = a.Code,
                    OriginName = a.Origin.Name,
                    OriginNameAr = a.Origin.NameAr,
                   AssetPeriorityName=a.AssetPeriority!=null? a.AssetPeriority.Name:"",
                   AssetPeriorityNameAr = a.AssetPeriority != null ?a.AssetPeriority.NameAr:"",

                    ECRIName = a.ECRIS.Name,
                    ECRINameAr = a.ECRIS.NameAr,
                    Model = a.ModelNumber
            ...
            {
                list = query.Select(a => new IndexMasterAssetVM.GetData
                {
                    Name = a.Name,
                    NameAr = a.NameAr,
                    BrandName = a.Brand.Name,
